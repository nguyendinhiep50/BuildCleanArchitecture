...
namespace BuildCleanArchitecture.Application.Books.Commands
{
    ...
    internal class UpdatBookCommandHandler : IRequestHandler<UpdateBookCommand, ResponseModel<bool>>
    {
        private readonly IApplicationDbContext _context;
        private readonly IMapper _mapper;

        public UpdatBookCommandHandler(IApplicationDbContext context,
            IMapper mapper)
        {
            _context = context;
            _mapper = mapper;
        ...
        public async Task<ResponseModel<bool>> Handle(UpdateBookCommand request, CancellationToken cancellationToken)
        {
            ...
            var book = await _context.Books.AsTracking().FirstOrDefaultAsync(x => x.Id == id);
